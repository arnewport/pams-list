...
const PatientModal = ({ show, handleClose, patient, onArchive, onUpdate, userRole, userId }) => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (acceptedInterest) {
              ...
              setToastMessage("This patient was accepted by another user. You cannot accept the patient at this time. If the patient is placed, this patient will be removed from your list. If the placement does not occur, then you will be able to accept the patient again.");
              setShowToast(true);
            } else if (data.some(interest => interest.status === 'accepted' && interest.marketerId === userId)) {
              setToastMessage("You have accepted this patient. Let Pam will remove this patient from your list once their placement has been confirmed.");
              setShowToast(true);
            }
  ...
  const handleArchive = async () => {
    if (isEditing) {
      try {
        ...
        setToastMessage('Failed to update the patient. Please try again.');
        setShowToast(true);
        return;
      }
    }
    try {
      ...
      setToastMessage('Error archiving the patient: ' + error);
      setShowToast(true);
    }
  };
  ...
  const handleInterested = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        setToastMessage('Interest registered successfully');
        setShowToast(true);
      } else {
        setToastMessage('Failed to register interest');
        setShowToast(true);
      }
    } catch (error) {
      console.error('Error registering interest:', error);
      setToastMessage('Error registering interest: ' + error);
      setShowToast(true);
    }
  };
  ...
  const handleAccept = async () => {
    try {
      ...
      setToastMessage('Patient accepted successfully!');
      setShowToast(true);
    } catch (error) {
      console.error('Error accepting the patient:', error);
      setToastMessage('Failed to accept the patient. Please try again.');
      setShowToast(true);
    }
  };
  ...
  const handleReject = async () => {
    try {
      ...
      setToastMessage('Patient rejected successfully!');
      setShowToast(true);
    } catch (error) {
      console.error('Error rejecting the patient:', error);
      setToastMessage('Failed to reject the patient. Please try again.');
      setShowToast(true);
    }
  };
  ...
  const handleConfirmPlacement = async () => {
    try {
      ...
      handleArchive();
      setToastMessage('The placement has been confirmed.');
      setShowToast(true);
    } catch (error) {
      console.error('Error confirming patient placement:', error);
      setToastMessage('Failed to confirm patient placement. Please try again.');
      setShowToast(true);
    }
  };
